...
describe("all should pass", () => {
  let slug: string;
  let longurl = "www.testurl.com";
  afterAll(async () => {
    const response = await request(baseUrl).delete(`/delete/${longurl}`);
  });
  describe("POST /encode - shorten a long url", () => {
    it("should return 200", async () => {
      let correctBody = { longurl };

      const response = await request(baseUrl).post("/encode").send(correctBody);
  ...
  describe("GET /decode - retrieve the longurl of a slug", () => {
    ...
    it("should return 200 if the slug is found", async () => {
      const response = await request(baseUrl).get(`/decode/${slug}`);
      let body = response.body;
      let data = body.data;
      expect(response.statusCode).to.equal(200);
